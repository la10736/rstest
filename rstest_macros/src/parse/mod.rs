...
use self::{expressions::Expressions, sys::DefaultSysEngine, vlist::ValueList};
    ...
    pub(crate) trait SysEngine {
        ...
        fn glob(&self, pattern: &str) -> Result<Vec<PathBuf>, String> {
            ...
            let globs = glob(pattern)
                .map_err(|e| format!("glob failed for whole path `{pattern}` due {e}"))?;
            globs
                .into_iter()
        ...
        fn read_file(&self, path: impl AsRef<std::path::Path>) -> Result<String, String> {
            let path = path.as_ref();
            std::fs::read_to_string(path)
                .map_err(|e| format!("failed to read file `{}` due {e}", path.to_string_lossy()))
        }
    }
...
pub(crate) trait ExtendWithFunctionAttrs {
    fn extend_with_function_attrs<S: sys::SysEngine>(
        &mut self,
        item_fn: &mut ItemFn,
