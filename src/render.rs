...
use syn::{parse_quote, FnArg, Generics, Ident, ItemFn, ReturnType, Stmt};
...
fn render_partial_impl(
    ...
    let generics = generics_clean_up(
        ...
        fixture_inputs.iter().take(n),
        &output,
    );
    let where_clause = &generics.where_clause;

    let to_resolve_args = fixture_inputs
        .iter()
        .skip(n)
...
mod test {
    ...
    mod fn_test_should {
        ...
        fn add_return_type_if_any() {
            let ast: ItemFn =
                "fn function(mut fix: String) -> Result<i32, String> { Ok(42) }".ast();
    ...
    mod cases_should {
        ...
        mod matrix_cases_should {
            ...
            fn create_a_module_named_as_test_function() {
                let item_fn =
                    parse_str::<ItemFn>("fn should_be_the_module_name(mut fix: String) {}")
                        .unwrap();
                let data = into_rstest_data(&item_fn);
